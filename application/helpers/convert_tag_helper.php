...
function convert_multi_question($data){
	$complete_data = null;
	$complete_data = str_replace('<tab>', '&nbsp;&nbsp;&nbsp;&nbsp;', $data);
	$complete_data = str_replace('[____]', '<button>[<font onclick="undo_answer(event)" ondrop="drop(event)" ondragover="allowDrop(event)" color="red">____</font>]</button>', $complete_data);
	return $complete_data;
}
